...
Outputs (written to OUTDIR, default ./golden next to this script; one test case
per "block"):
...
PRIVATE instructor tool (not shipped to students). The PUBLIC patterns are the
committed copies under hw/hw3-synth/00_TB/golden/. Regenerate HIDDEN grading
patterns with a different --seed, e.g.:
  python3 gen_golden.py --seed 12345 --outdir /tmp/hidden_golden
"""
import argparse
import os
import random
...
# ----------------------------- test cases -----------------------------
def make_cases(seed=20260603):
    rng = random.Random(seed)   # deterministic; 20260603 == the committed PUBLIC set
...
def main():
    ...
    ap.add_argument("--seed", type=int, default=20260603,
                    help="RNG seed (20260603 reproduces the committed PUBLIC set; "
                         "use a different value for HIDDEN grading patterns)")
    ap.add_argument("--outdir", default=os.path.join(here, "golden"),
                    help="output directory (default: ./golden next to this script)")
    args = ap.parse_args()
